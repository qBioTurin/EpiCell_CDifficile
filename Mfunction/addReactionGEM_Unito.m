...
function [newmodel, HTABLE] = addReactionGEM_Unito(model, rxns, rxnNames, rxnFormulas, rev, lb, ub, subSystems, grRules, rules, genes, HTABLE)
...
for i = 1:length(rev)
    ...
    newmodel.subSystems{nRxn,1} = subSystems{i};
    newmodel.grRules(nRxn,1) = grRules(i,1);
    newmodel.rules(nRxn,1) = rules(i,1);
    newmodel.c(nRxn,1) = 0;
    newmodel.rxnNames{nRxn,1} = char(rxnNames(i,1));
    % Adding the new columns in S
    newmodel.S(:,nRxn) = zeros(length(newmodel.mets), 1);
    ...
    while j <= nComponents
        ...
        elseif str2double(parsing{j}) > 0
            ...
            if j>1
                % if j =1 it means that it is the first met withuot coefficient
                rxnCoeff = str2double(parsing{j-1}); % era j-1
                if isnan(rxnCoeff)
                    rxnCoeff=1;
                    rxnNames(i,1);
                end
