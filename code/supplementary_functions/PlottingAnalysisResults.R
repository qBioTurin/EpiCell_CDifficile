...
PlottingPanorama <- function(Exper, Condition, tag, pdf.name) {
  ...
  # Read in data
  subtrace = readr::read_csv(paste0(results.dir, "CDiff_", tag, "_", Condition, "_", 
                                    Exper, "/SubTrace", tag, Condition, ".csv"))[-1]
  subflux = readr::read_csv(paste0(results.dir, "CDiff_", tag, "_", Condition, "_", 
                                   Exper, "/SubFlux", tag, Condition, ".csv"))[-1]
  subbounds = readr::read_csv(paste0(results.dir, "CDiff_", tag, "_", Condition, "_", 
  ...
  D = ggplot(Drug, aes(Time, Marking)) + theme_bw() +
    geom_line(size = 0.8) + geom_line(color = colConFirst) +
    ...
    geom_vline(xintercept = 40, linetype = "dotted", color = "darkblue", lwd = 0.70) +
    geom_vline(xintercept = 48, linetype = "dotted", color = "darkblue", lwd = 0.70) +
    geom_hline(yintercept = 5800, linetype = "dashed", color = "darkgrey", lwd = 0.5) +
  ...
  Hc = ggplot(pheme_c, aes(Time, Marking)) + theme_bw() +
    geom_line(size = 0.8) + geom_line(color = colConFirst) +
    labs(x = "Time (h)", y = "Intracell. heme concentration (pmol)",
         title = paste("Model dynamics ", "(", tag, " | ", Condition, ")", sep = ""),
         subtitle = paste("Place: ", pheme_c[1, 2], " (pmol)", sep = "")) +
    theme(axis.text = element_text(size = 14), 
          axis.title=element_text(size = 14),
          plot.title = element_text(size = 16),
          plot.subtitle = element_text(size = 14)) + 
  ...
  He = ggplot(pheme_e, aes(Time, Marking)) + theme_bw() +
    geom_line(size = 0.8) + geom_line(color = colConFirst) +
    ...
    geom_vline(xintercept = 40, linetype = "dotted", color = "darkblue", lwd = 0.70) +
    geom_vline(xintercept = 48, linetype = "dotted", color = "darkblue", lwd = 0.70) +
    geom_hline(yintercept = 1e+06, linetype = "dashed", color = "darkred", lwd = 0.5) +
    annotate("text", 12, 0.95e+06, label = "Hemorrhagic baseline", vjust = 0.5, color = "black", size = 3) +
    scale_y_continuous(label = scientific_10)
  
  A = ggplot(AA, aes(Time, (Marking/(6.022e20/6.022e08))*1000, color = Places)) + theme_bw() +
    geom_line(size = 0.8) +
  ...
  # Txn = ggplot(Toxin, aes(Time, Marking)) + theme_bw() +
  #   geom_line(size = 0.8) + geom_line(color = colConFirst) +
  #   labs(x = "Time (h)", y = "Toxin concentration (pmol)",
  #        title = paste0("Model dynamics ", "(", tag, " | ", Condition, ")"),
  #        subtitle = paste0("Place: ", Toxin[1, 2], " (pmol)")) +
  #   theme(axis.text = element_text(size = 14), 
  #         axis.title=element_text(size = 14),
  #         plot.title = element_text(size = 16),
  #         plot.subtitle = element_text(size = 14)) + 
  ...
  bH = ggplot(BoundsSinkHeme, aes(Time, Flux, linetype = Reaction)) + theme_bw() +
    geom_line(size = 0.8) + geom_line(color = colConFirst) +
    labs(x = "Time (h)", y = "sink_pheme(c) Lb & Ub (mmol/gDW*h)",
         title = paste0("Model dynamics ", "(", tag, " | ", Condition, ")"),
         subtitle = paste0("Double bounded flux (Lb ≤ v ≤ Ub)")) +
    theme(axis.text = element_text(size = 14), 
          axis.title=element_text(size = 14),
